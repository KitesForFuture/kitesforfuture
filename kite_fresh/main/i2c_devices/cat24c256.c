...
void write2EEPROM(float number, int address){
	...
	i2c_send_16bitDataAddress(0x50, address, number, 4);
}

float readEEPROM(int address){
	//i2c_send(bus_no(1 for bmp and cat), chip_address, data_addr, data, length);
	i2c_receive_16bitDataAddress(0x50, address); // THIS IS A DIRTY HACK, because when data_addr != 0 and a write has just taken place, a read returns garbage. TODO: Why?
	return i2c_receive_16bitDataAddress(0x50, address);
}
